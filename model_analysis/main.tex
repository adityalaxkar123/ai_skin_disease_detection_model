...
We identify several directions to extend and strengthen this work:

\begin{itemize}
    \item \textbf{Expanded taxonomy:} Increase the number of labelled dermatological conditions (including rare classes) to move toward clinically comprehensive coverage.
    \item \textbf{Larger, diverse datasets:} Curate multi-center datasets encompassing varied skin tones, imaging devices, and lighting conditions to reduce domain shift.
    \item \textbf{Prospective external validation:} Conduct multi-site prospective studies and reader studies with dermatologists to evaluate clinical utility and robustness.
    \item \textbf{Explainability and trust:} Integrate saliency methods such as Grad-CAM and perform clinician-in-the-loop evaluations to improve interpretability and trust.
    \item \textbf{Semi-supervised and active learning:} Leverage unlabeled data and active annotation strategies to improve performance on underrepresented classes.
    \item \textbf{Calibration and uncertainty:} Investigate temperature scaling, deep ensembles, and Bayesian approximations for reliable confidence estimates in deployment.
    \item \textbf{Edge optimization and privacy:} Apply model compression, quantization, and knowledge distillation for on-device inference; explore federated learning for privacy-preserving training.
\end{itemize}
